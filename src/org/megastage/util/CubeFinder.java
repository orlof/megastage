...
public class CubeFinder {
        ...
        for(int x=0; x < map.xsize; x += CHUNK_SIZE) {
            for(int y=0; y < map.ysize; y += CHUNK_SIZE) {
                for(int z=0; z < map.zsize; z += CHUNK_SIZE) {
                    ...
                    boolean mh = checkBlock(blockCenter, radius, rayPos, rayDir);
                }
            }
        }
    }
    ...
    private static boolean checkBlock(Vector3d blockCenter, double blockRadius, Vector3d rayPos, Vector3d rayDir) {
